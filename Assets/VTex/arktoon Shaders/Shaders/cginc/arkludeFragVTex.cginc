...
{   
    ...
    float LINESPACE = 15;
    float CHARSPACE = 6;
    float2 fontxy = float2(floor(((xy.x % 1.0)) * (FONTWIDTH + CHARSPACE)), floor((1.0 - (xy.y % 1.0)) * (FONTHEIGHT + LINESPACE)));
    fontxy = IS_HORIZONTAL * fontxy + (1 - IS_HORIZONTAL) * float2(FONTWIDTH - fontxy.y, fontxy.x);
    float fontcolindex[3] = {   unicode * 32 + clamp(fontxy.y - 1.0, 0.0, FONTHEIGHT - 1.0),
                                unicode * 32 + fontxy.y,
                                unicode * 32 + clamp(fontxy.y + 1.0, 0.0, FONTHEIGHT - 1.0)};
...
float4 frag(VertexOutput i) : COLOR {
    ...
    //_MainTex_var = VTex(i.uv0);
    float4 vtex = VTex(i.uv0);
    float3 Diffuse = (vtex.rgb*REF_COLOR.rgb);

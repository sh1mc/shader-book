...
Shader "arktoon/AlphaCutout1" {
    Properties {
        [MaterialToggle] _OnlyMirror ("OnlyMirror", Float ) = 0
        // Double Sided
    ...
    SubShader {
        ...
        Pass {
            ...
            #include "cginc/arkludeFrag1.cginc"
            ENDCG
        }
        Pass {
            Name "FORWARD_DELTA"
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                // kagami
                float r = _MainTex_var.r;
                float isMirror = step(UNITY_MATRIX_P._m22, 0.000001);
                _MainTex_var.r = isMirror * _MainTex_var.b + (1 - isMirror) * _MainTex_var.r;
                _MainTex_var.g = isMirror * r + (1 - isMirror) * _MainTex_var.g;
                clip((_MainTex_var.a * _Color.a) - _CutoutCutoutAdjust);
                SHADOW_CASTER_FRAGMENT(i);
                clip(0.5 - _OnlyMirror * (1 - isMirror));
            }
            ENDCG
        }
    }
